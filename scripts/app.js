// SET URL SERVICE APP
var setUrl = "http://mediatel.co.rs/admin/";
...
var tbBookmark = localStorage.getItem("tbBookmark");//Retrieve the stored data
	...
	//if (tbBookmark == 'undefined' || tbBookmark.length == 0 || tbBookmark == null) { 
...
function getCategory(){
	...
	var url = 'http://mediatel.co.rs/cms/category';
...
// GET CATEGORY DATA JQUERY MOBILE	
/*
function getCategory(){
	$('#list-category').empty();
//	var url = urlService+'service/get_category';
	var url = 'http://mediatel.co.rs/cms/json';
...
function getList(id){
	...
	$('#list-all').empty();
	//var url = urlService+'http://mediatel.co.rs/cms/subcategory?subcategory='+id;
	var url = 'http://mediatel.co.rs/cms/subcategory?subcategory='+id;
	getRequest(url, function(data) {
        var data = JSON.parse(data.responseText);         
        for (var i = 0; i < data.length-1; i++) {
		...
		getRequest(url, function(data) {
       		var data = JSON.parse(data.responseText);
				...
				$('#list-bookmark').append('<li>'+
					...
					'</a>'+
					'<span class="delBookmark" rel="'+data[i]['list_id']+'">DEL</span>'+
    				'</li>').listview('refresh');
			}
		});
	}
	// Ispitaj da li ima bookmarkova pre neko sto iscrtas 
	if (tbBookmark !== 'undefined' && tbBookmark.length > 0) { 
		$('#list-bookmark').append('<li><span class="delAllBookmarks">Obrisi sve favorite</span></li>').listview('refresh');
	}
	...
	getRequest(url, function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
			...
			if(!data[i]['list_image']){
				...
				var img  = 'upload/images/'+ data[i]['list_image'];
				var img1 = 'upload/images/'+ data[i]['list_image'];
				var img2 = 'upload/images/'+ data[i]['list_image'];
			}
			
			$('#btn-bookmarks').html('<img src="images/buttom.png" border="0" style="position:absolute; margin-top:-17px;">');
			$('#name').html(data[i]['list_name']);
			//$('#image').html('<div id="slider" class="swipe"><div class="swipe-wrap"><div><img src="'+setUrl+''+ img +'" width="250px" height="200px" /></div><div><img src="'+setUrl+''+ img1 +'" width="250px" height="200px" /></div>    <div><img src="'+setUrl+''+ img2 +'" width="250px" height="200px" /></div></div></div>');
			$('#image').html('<img src="'+setUrl+''+ img +'" width="100%" height="100%" />');
...
function delAllBookmarks(){		
	//localStorage.clear();
	localStorage.removeItem('tbBookmark');
	localStorage.removeItem('id');
	alert("Obrisani svi favoriti.");
	$('#list-bookmark').html('');
	return true;
...
$(document).on('click', '.delAllBookmarks', function() {
	delAllBookmarks();
});
...
$(document).on('click', '#btn-bookmark', function() {
	...
	//tbBookmark.push(client);
	tbBookmark.unshift(client);
	
	localStorage.setItem("tbBookmark", JSON.stringify(tbBookmark)); 
	//localStorage.setItem(JSON.stringify(tbBookmark), JSON.stringify(tbBookmark)); 
	alert("Objekat je ubacen u bookmark.");
	return true;
});	
